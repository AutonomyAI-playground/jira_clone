...
const BacklogFilters = ({ projectUsers, defaultFilters, filters, mergeFilters }) => {
  const { searchTerm, userIds, myOnly, recent } = filters;

  const areFiltersCleared = !searchTerm && userIds.length === 0 && !myOnly && !recent;

  return (
    <Filters data-testid="backlog-filters">
      <SearchInput
        icon="search"
        value={searchTerm}
        ...
        {projectUsers.map(user => (
          <AvatarIsActiveBorder key={user.id} isActive={userIds.includes(user.id)}>
            <StyledAvatar
              ...
              name={user.name}
              // XOR toggles the user: adds if not present, removes if present
...
BacklogFilters.propTypes = propTypes;
